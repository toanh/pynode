...
**PyNode** is a Python library for visualizing Graph Theory. It can be used to develop algorithm prototypes, or to demonstrate how algorithms work in a visual, interactive way. The official website can be found <a href="https://alexsocha.github.io/pynode">here</a>.
<br><br>
...
* **pynode_core.py** - Handles the internal functions of the API, and acts as the bridge between pynode_graphlib.py and graph_api.js.
...
* **index.html** - The main page, which includes the editor, console, and output window. Also provides documentation for all features.
...
A browser test suite lives in [/tests](tests). It drives a locally installed Chrome (or
Edge) with Playwright and starts its own web server, so no Python and no manually started
server are needed:
...
See [tests/README.md](tests/README.md) for what each file covers and how to debug a
failure.
